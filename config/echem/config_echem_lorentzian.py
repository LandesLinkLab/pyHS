...
# Number of peaks
args['NUM_PEAKS'] = 1  # 1: Single peak, 2: Two peaks, 3+: Multiple peaks

# ⚠️ CRITICAL: Initial guesses (MUST PROVIDE FOR EACH PEAK)
args['PEAK_POSITION_INITIAL_GUESS'] = [700.0]  # List of positions in nm (REQUIRED!)
...
# Constraints
args['PEAK_WIDTH_MAX'] = [100.0]  # Max FWHM per peak
                                  # Single value: applies to all peaks
                                  # List: [80, 120] for different max per peak

args['PEAK_POSITION_TOLERANCE'] = [100.0]  # Allowed deviation from initial guess (nm)
                                            # None: No constraint
                                            # Single value: applies to all peaks
